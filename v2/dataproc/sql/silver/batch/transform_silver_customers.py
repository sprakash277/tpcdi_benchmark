# Dataproc: transform bronze_customer_mgmt -> silver_customers
...
bronze_df = spark.table(f"{database}.bronze_customer_mgmt").filter(col("_batch_id") == batch_id)
customer_df = None
try:
...
silver_customers.write.format("delta").mode("overwrite").saveAsTable(f"{database}.silver_customers")
