# Dataproc: transform bronze_customer_mgmt -> silver_accounts
# Expects globals: spark, database, batch_id (set by run_tpcdi_batch.py)
from pyspark.sql.functions import col, lit, when, to_timestamp, coalesce, trim
from pyspark.sql.types import TimestampType

bronze_accounts_df = spark.table(f"{database}.bronze_customer_mgmt").filter(col("_batch_id") == batch_id)
account_df = None
...
silver_accounts.write.format("delta").mode("overwrite").saveAsTable(f"{database}.silver_accounts")
