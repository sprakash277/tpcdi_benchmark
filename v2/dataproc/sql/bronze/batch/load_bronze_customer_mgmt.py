# Dataproc: load bronze_customer_mgmt from Batch1/CustomerMgmt.xml
# Expects globals: spark, database, full_raw_path, batch_id (set by run_tpcdi_batch.py); xml_format optional
xml_format = (globals().get("xml_format") or "com.databricks.spark.xml").strip() or "xml"
xml_path = f"{full_raw_path}/Batch1/CustomerMgmt.xml"
...
if not success or df is None:
    raise RuntimeError(
        ...
        "Ensure spark-xml JAR is on classpath (e.g. com.databricks:spark-xml_2.12:0.18.0)."
    )
...
spark.sql(f"DROP TABLE IF EXISTS {database}.bronze_customer_mgmt")
df_bronze.write.format("delta").mode("overwrite").saveAsTable(f"{database}.bronze_customer_mgmt")
