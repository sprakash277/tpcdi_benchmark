...
WITH incoming_customers AS (
    SELECT 
        ...
        CAST(split(raw_line, '\\|')[2] AS BIGINT) AS customer_id,  -- Skip CDC_FLAG, CDC_DSN
        ...
        split(raw_line, '\\|')[0] AS cdc_flag,  -- I=Insert, U=Update, D=Delete
        CAST(split(raw_line, '\\|')[1] AS TIMESTAMP) AS cdc_dsn,  -- Change timestamp
...
WITH incoming_trades AS (
    SELECT 
        CAST(split(raw_line, '\\|')[2] AS BIGINT) AS trade_id,  -- Skip CDC_FLAG, CDC_DSN
