...
# Incremental load bronze_daily_market from Batch{N}/DailyMarket.txt
...
spark.sql(f"""
INSERT INTO {catalog}.{schema_name}.bronze_daily_market (raw_line, _batch_id, _load_timestamp, _source_file)
SELECT 
    value AS raw_line,
    {batch_id} AS _batch_id,
    current_timestamp() AS _load_timestamp,
    'DailyMarket.txt' AS _source_file
FROM read_files('{full_raw_data_path}/Batch{batch_id}/DailyMarket.txt', format => 'text', lineSep => '\\n')
